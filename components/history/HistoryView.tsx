...
import { describeFilter, filterHistory, summarizeHistory } from "@/lib/history";
  ...
  return (
    <div className="flex flex-col items-center rounded-2xl border border-border-subtle bg-surface px-6 py-14 text-center shadow-card">
      ...
      <p className="mt-1 max-w-xs text-sm text-muted">
        Nothing was recorded for {label}. Try a wider range, or record an expense
        on the Tracker.
      </p>
    </div>
  );
 ...
 * The history screen.
 *
 * Reads sealed snapshots from the provider — never today's budget — so the
 * figures shown (and exported) stay true to what was recorded at the time.
 */
export function HistoryView() {
  const { hydrated, history } = useTracker();
